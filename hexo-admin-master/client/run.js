...
class DataEditor {
  constructor(node, id = null) {
    ...
    this.matchesFetcher = new DataFetcher(this.fetchMatches.bind(this));
    this.resultsFetcher = new DataFetcher(this.fetchResults.bind(this));
    this.continueEditing = localStorage.getItem('continueEditing') === 'true';
  }
  ...
  getWinningTeams() {
    const results = this.resultsFetcher.data || [];
    const winningTeams = new Set();

    results.forEach(result => {
      if (result.team1Score > result.team2Score) {
        winningTeams.add(result.team1);
      } else if (result.team2Score > result.team1Score) {
        winningTeams.add(result.team2);
      }
    });

    return Array.from(winningTeams);
  }
  ...
  updateTeamOptions() {
    ...
    const tournamentRound = tournamentRoundSelect.value;
    const isFirstRound = tournamentRound === '1';

    // Si c'est un tournoi et pas le premier tour, on ne montre que les équipes gagnantes
    const availableTeams = isTournament && !isFirstRound ? this.getWinningTeams() : 
      (this.teamsFetcher.data || []).map(team => team.teamName);
  ...
  updateView() {
    if (this.dataFetcher.loading || this.teamsFetcher.loading || this.stadesFetcher.loading || 
        this.matchesFetcher.loading || this.resultsFetcher.loading) {
      this.node.innerHTML = '<div class="loading">Chargement...</div>';
      return;
    }

    if (this.dataFetcher.error || this.teamsFetcher.error || this.stadesFetcher.error || 
        this.matchesFetcher.error || this.resultsFetcher.error) {
      this.node.innerHTML = `<div class="error">${this.dataFetcher.error || this.teamsFetcher.error || 
        this.stadesFetcher.error || this.matchesFetcher.error || this.resultsFetcher.error}</div>`;
      return;
    }
      ...
      <div class="data-editor">
        ...
        <form id="data-form">
          ...
          <div class="form-group tournament-fields" style="display: none;">
            <label for="tournamentRound">Tour du tournoi</label>
          ...
          <div class="form-group">
            ...
            <input type="datetime-local" id="matchDate" name="matchDate" value="${this.parseDate(data.matchDate) || ''}" required>
          </div>
          <div class="form-group">
            ...
            <select id="location" name="location" required>
              ...
              ${stades.map(stade => `
                <option value="${stade.stadeName}" 
                  ${data.location === stade.stadeName ? 'selected' : ''}>
    ...
    const team1Select = document.getElementById('team1');
    const matchTypeSelect = document.getElementById('matchType');
    const tournamentFields = document.querySelector('.tournament-fields');
    const continueEditingCheckbox = document.getElementById('continueEditing');

    // Gestion de l'affichage des champs du tournoi
    matchTypeSelect.addEventListener('change', () => {
      tournamentFields.style.display = matchTypeSelect.value === 'tournament' ? 'block' : 'none';
    ...
    tournamentFields.querySelector('#tournamentRound').addEventListener('change', () => this.updateTeamOptions());
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const data = {
        ...
        matchDate: this.formatDate(formData.get('matchDate')),
